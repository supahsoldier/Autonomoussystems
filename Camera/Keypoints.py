...
def publish(client, topic, x, y, rotation):
    data = {
        "x": adjusted_cell_x,
        "y": adjusted_cell_y,
        "z": 0,
        ...
        for i, box in enumerate(boxes):
            ...
            if confidence >= 0.8:
                ...
                if classNames[cls] == 'ChariotY':
                    publish(client, "chariot/5/position", adjusted_cell_x, adjusted_cell_y,rotation)
                    publishabsolute(client, "chariot/5/absolute")
                    publishcenter(client, "chariot/5/center", cell_center)
                elif classNames[cls] == 'ChariotB':
                    publish(client, "chariot/6/position", adjusted_cell_x, adjusted_cell_y)
